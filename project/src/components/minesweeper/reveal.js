export const revealed = (arr, x, y, newNonMinesCount) => {
  //koopia gridist ja kuhu klikiti
  console.log(arr[x][y]);
  if (arr[x][y].revealed) {
  ...
  let flipped = []; //keep track of zeros
  flipped.push(arr[x][y]); //first item is where you clicked
  while (flipped.length !== 0) {
    let single = flipped.pop(); //esimene element valja et saaks selle naabreid vorrelda
    ...
    if (single.value !== 0) {
      break;
    } // if not zero is found stop revealing
